        ...
        private void EmitDuckMethod(StringBuilder sb, List<CandidateModel> items)
        {
            ...
            foreach (var candidate in duckCandidates)
            {
                ...
                if (candidate.TargetIsInterface && _possibleMatches.TryGetValue(iface, out var matches))
                {
                    ...
                    {
                        var local = $"c_{ui++}";
                        sb.AppendLine($"                    if (target.TryUnbox<{m.Fq}>(out var {local})) return (T)(object)new {ProxyFullName(m.Namespace, m.MinimalName, m.Fq, candidate.InterfaceMinimalName, candidate.InterfaceFq)}({local});");
                    }
                }
                ...
                sb.AppendLine($"                    return (T)(object)new {ProxyFullName(candidate.UnderlyingNamespace, candidate.UnderlyingMinimalName, candidate.UnderlyingFq, candidate.InterfaceMinimalName, candidate.InterfaceFq)}(({candidate.UnderlyingFq})target);");
                sb.AppendLine("                }");
            }
